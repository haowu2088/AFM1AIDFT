...
def predict(accelerator, data_loader, model, logger, config):
    ...
    for batch_idx, (inputs, target, cif_id) in enumerate(data_loader):           # Wu: notice
        ...
        with torch.no_grad():
            ...
            label1_probs = output.detach().cpu()[:, 1]  # [b, ]    # This moves the tensor to the CPU. # each column corresponds to a class probability
...
def main():
    ...
    dataset = PredictCIFData(root_dir      = config['root_dir'],
    ...
    cif_ids, cif_probs = predict(accelerator  = accelerator,
                                 model        = model,
