    ...
    for epoch in range(config['start_epoch'], config['start_epoch']+config['epochs']):
        ...
        for batch_idx, (inputs, target, cif_id) in enumerate(train_loader):
            ...
            if accelerator.is_main_process and (epoch % config['log_epoch_freq'] == 0 or epoch == 1) and batch_idx % config['log_batch_freq'] == 0:
                logger.info('[Pretrain] Epoch [{}/{}] [{}/{}]\t BT {:.3f} ({:.3f})\t DT {:.3f} ({:.3f})\t Loss {:.4e} ({:.4e})\t [Save]'\
                            .format(epoch, config['start_epoch']+config['epochs']-1, batch_idx, len(train_loader), batch_time.val, batch_time.avg, data_time.val, data_time.avg, losses.val, losses.avg))
        
        # Save model state, separating batch-based logging from epoch-based saving
        if accelerator.is_main_process and epoch % config['save_epoch_freq'] == 0:
            ...
            logger.info('Model saved: [{}]-{}epoch'.format(config['pre_ckpt_path'], epoch))  # Log saving action        

        scheduler.step()  # adjusts the learning rate over time
...
def main():
    ...
    dataset = PretrainCIFData(root_dir      = config['root_dir'],
    ...
    # Wu: added by me to display total number of data points and number of batches
    total_data_points = len(dataset)
    num_batches = len(train_loader)
    if accelerator.is_main_process:
        logger.info(f'Total number of data points: {total_data_points}')
    ...
    model = CrystalGraph(crystal_gnn_config  = crystal_gnn_config,
    ...
    pretrain_loss = pretrain(accelerator  = accelerator,
                             model        = model,
