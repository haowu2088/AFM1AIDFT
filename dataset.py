...
class PredictCIFData(Dataset):
    ...
    def __getitem__(self, idx):
        atom_fea, nbr_fea, nbr_fea_idx, degree, target, cif_id = \
            self.dataset_cand[0][idx], \
            self.dataset_cand[1][idx], \
            self.dataset_cand[2][idx], \
            self.dataset_cand[3][idx], \
            self.dataset_cand[4][idx], \
            self.dataset_cand[5][idx]
        return (atom_fea, nbr_fea, nbr_fea_idx, degree), target, cif_id
    


# Wu: the next 5 classes are for the 5 datasets I want to extraxt encoded features
# Even though, it seems redundant because we already have candidate, positive, negative datasets above, 
# but to be clean, simple, and straighforward, let us just write down 5 explicitly.
class ExtractCandCIFData(Dataset):
    def __init__(self, root_dir, processed_dir, logger, max_num_nbr, radius, dmin, step):
        self.root_dir                 = root_dir
        self.processed_dir_cand       = osp.join(self.root_dir, processed_dir['candidate'])    # Wu: notice
        self.logger                   = logger
        self.max_num_nbr, self.radius = max_num_nbr, radius
        assert os.path.exists(root_dir), 'root_dir does not exist!'
...
class ExtractNegaCIFData(Dataset):
    def __init__(self, root_dir, processed_dir, logger, max_num_nbr, radius, dmin, step):
        self.root_dir                 = root_dir
        self.processed_dir_nega       = osp.join(self.root_dir, processed_dir['negative'])    # Wu: notice
        self.logger                   = logger
        self.max_num_nbr, self.radius = max_num_nbr, radius
        assert os.path.exists(root_dir), 'root_dir does not exist!'
        ...
        self.dataset_nega = self._check_data()
        self.len_nega = len(self.dataset_nega[0])
    ...
    def __len__(self):
        return self.len_nega


    def __getitem__(self, idx):
        atom_fea, nbr_fea, nbr_fea_idx, degree, target, cif_id = \
            self.dataset_nega[0][idx], \
            self.dataset_nega[1][idx], \
            self.dataset_nega[2][idx], \
            self.dataset_nega[3][idx], \
            self.dataset_nega[4][idx], \
            self.dataset_nega[5][idx]
        return (atom_fea, nbr_fea, nbr_fea_idx, degree), target, cif_id




class ExtractPosiCIFData(Dataset):
    def __init__(self, root_dir, processed_dir, logger, max_num_nbr, radius, dmin, step):
        self.root_dir                 = root_dir
        self.processed_dir_posi       = osp.join(self.root_dir, processed_dir['positive'])    # Wu: notice
        self.logger                   = logger
        self.max_num_nbr, self.radius = max_num_nbr, radius
        assert os.path.exists(root_dir), 'root_dir does not exist!'
        ...
        self.dataset_posi = self._check_data()
        self.len_posi = len(self.dataset_posi[0])
    ...
    def __len__(self):
        return self.len_posi


    def __getitem__(self, idx):
        atom_fea, nbr_fea, nbr_fea_idx, degree, target, cif_id = \
            self.dataset_posi[0][idx], \
            self.dataset_posi[1][idx], \
            self.dataset_posi[2][idx], \
            self.dataset_posi[3][idx], \
            self.dataset_posi[4][idx], \
            self.dataset_posi[5][idx]
        return (atom_fea, nbr_fea, nbr_fea_idx, degree), target, cif_id




class ExtractTNegaCIFData(Dataset):
    def __init__(self, root_dir, processed_dir, logger, max_num_nbr, radius, dmin, step):
        self.root_dir                 = root_dir
        self.processed_dir_tnega       = osp.join(self.root_dir, processed_dir['tnegative'])    # Wu: notice
        self.logger                   = logger
        self.max_num_nbr, self.radius = max_num_nbr, radius
        assert os.path.exists(root_dir), 'root_dir does not exist!'

        id_prop_tnega_file = os.path.join(self.root_dir, 'id_prop_20.csv')
        atom_init_file    = os.path.join(self.root_dir, 'atom_init.json')
        assert os.path.exists(id_prop_tnega_file), 'id_prop_20.csv does not exist!'
        assert os.path.exists(atom_init_file), 'atom_init.json does not exist!'

        with open(id_prop_tnega_file) as f:
            reader = csv.reader(f)
            self.id_prop_tnega_data = [row for row in reader]

        self.ari = AtomCustomJSONInitializer(atom_init_file)
        self.gdf = GaussianDistance(dmin=dmin, dmax=self.radius, step=step)

        self.dataset_tnega = self._check_data()
        self.len_tnega = len(self.dataset_tnega[0])


    def _check_data(self):
        if not osp.exists(self.processed_dir_tnega):
            self.logger.info('Process...')
            process_data(ari           = self.ari,
                         gdf           = self.gdf,
                         id_prop_data  = self.id_prop_tnega_data,
                         radius        = self.radius,
                         max_num_nbr   = self.max_num_nbr,
                         root_dir      = self.root_dir,
                         save_dir      = self.processed_dir_tnega)
            
        dataset_tnega = load_data(self.processed_dir_tnega)
        return dataset_tnega


    def __len__(self):
        return self.len_tnega


    def __getitem__(self, idx):
        atom_fea, nbr_fea, nbr_fea_idx, degree, target, cif_id = \
            self.dataset_tnega[0][idx], \
            self.dataset_tnega[1][idx], \
            self.dataset_tnega[2][idx], \
            self.dataset_tnega[3][idx], \
            self.dataset_tnega[4][idx], \
            self.dataset_tnega[5][idx]
        return (atom_fea, nbr_fea, nbr_fea_idx, degree), target, cif_id
    



class ExtractTPosiCIFData(Dataset):
    def __init__(self, root_dir, processed_dir, logger, max_num_nbr, radius, dmin, step):
        self.root_dir                 = root_dir
        self.processed_dir_tposi       = osp.join(self.root_dir, processed_dir['tpositive'])    # Wu: notice
        self.logger                   = logger
        self.max_num_nbr, self.radius = max_num_nbr, radius
        assert os.path.exists(root_dir), 'root_dir does not exist!'

        id_prop_tposi_file = os.path.join(self.root_dir, 'id_prop_21.csv')
        atom_init_file    = os.path.join(self.root_dir, 'atom_init.json')
        assert os.path.exists(id_prop_tposi_file), 'id_prop_21.csv does not exist!'
        assert os.path.exists(atom_init_file), 'atom_init.json does not exist!'

        with open(id_prop_tposi_file) as f:
            reader = csv.reader(f)
            self.id_prop_tposi_data = [row for row in reader]

        self.ari = AtomCustomJSONInitializer(atom_init_file)
        self.gdf = GaussianDistance(dmin=dmin, dmax=self.radius, step=step)

        self.dataset_tposi = self._check_data()
        self.len_tposi = len(self.dataset_tposi[0])


    def _check_data(self):
        if not osp.exists(self.processed_dir_tposi):
            self.logger.info('Process...')
            process_data(ari           = self.ari,
                         gdf           = self.gdf,
                         id_prop_data  = self.id_prop_tposi_data,
                         radius        = self.radius,
                         max_num_nbr   = self.max_num_nbr,
                         root_dir      = self.root_dir,
                         save_dir      = self.processed_dir_tposi)
            
        dataset_tposi = load_data(self.processed_dir_tposi)
        return dataset_tposi


    def __len__(self):
        return self.len_tposi


    def __getitem__(self, idx):
        atom_fea, nbr_fea, nbr_fea_idx, degree, target, cif_id = \
            self.dataset_tposi[0][idx], \
            self.dataset_tposi[1][idx], \
            self.dataset_tposi[2][idx], \
            self.dataset_tposi[3][idx], \
            self.dataset_tposi[4][idx], \
            self.dataset_tposi[5][idx]
        return (atom_fea, nbr_fea, nbr_fea_idx, degree), target, cif_id