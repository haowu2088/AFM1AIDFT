...
class CrystalGraph(nn.Module):
    def __init__(self, crystal_gnn_config, head_output_dim, drop_rate, decoder_sample_size, device):
        super().__init__()
        self.encoder = CrystalGraphConvNet(orig_atom_fea_len=crystal_gnn_config['orig_atom_fea_len'],
                                           atom_fea_len=crystal_gnn_config['atom_fea_len'],
                                           nbr_fea_len=crystal_gnn_config['nbr_fea_len'],
                                           n_conv=crystal_gnn_config['n_conv'])
        self.decoder = GNNAutoDecoder(input_dim=crystal_gnn_config['orig_atom_fea_len'],
                                      ...
                                      lambda_loss1=1e-6,
                                      lambda_loss2=1)
        self.head = nn.Sequential(nn.Linear(crystal_gnn_config['atom_fea_len'], crystal_gnn_config['atom_fea_len']),
                                  nn.Dropout(p=drop_rate),
                                  nn.ReLU(),  # activation function that introduces non-linearity into the model
    ...
    def forward(self, atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx):
        ...
        encoded_features = self.encoder(atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx)   
        # feature = F.normalize(self.head(feature), dim=1)
        processed_features = self.head(encoded_features)                                              
        return processed_features                                                   

    def predict(self, atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx):
        features = self.forward(atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx)
        probabilities = self.softmax(features)
        return probabilities

    # Added by Wu 2024.06.25
